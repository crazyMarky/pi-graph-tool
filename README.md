...
- **输入框直接下任务**：在页面输入任务（如"帮我调研 React、Vue、Svelte 然后对比汇总"），服务器无头拉起 pi 执行，图实时生长，最终回答回显在页面——单窗口完整演示
- **点击节点看内容**：每个节点可点开，查看该节点的完整输入 prompt（含 `{{id}}` 注入后的内容）与产出文本（前 2000 字）、耗时/字数、节点档案（工具白名单/阈值/截断/目录）
- **实时**：节点按 Wave 分列生长、运行中节点蓝色脉冲带秒表、活动边流动虚线、波间屏障完成后变绿放行
- **历史**：切换任意一次已落盘的 run，支持按时序 4x 重放动画
- **粘贴回放**：把 `graph_run` 结果里的 `details` JSON 粘进来，离线重建视图（无需服务器）
- 语义可视化：隐式推断边（琥珀虚线）与显式边区分；违约→重试挽回（`重试挽回` 徽标）；失败级联（后代灰化 + 原因）

注意：trace 含各节点 prompt 与产出文本（产出截断至 2000 字），目录内容请勿外传。
